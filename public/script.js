document.addEventListener('DocumentLoaded', function() {
    ...
    const themeToggle = dovument.getElementById('themeToggle');
    ...
    if(saveTheme === 'dark' || (!savedTheme && prefersDark)) {
...
const menuToggle = document.getElementById('manuToggle');
...
const observer = new IntersectionObserver((entries, observer) => {
    entries.forEach(entry => {
       if (entry.isIntersecting)  {
        ...
        entry.target.classList.remove('opacity-100', 'translate-y-4');
...
const terminalContent = document.querySelector('terminal-container');
const commandSpan = document.querySelector('command-text');

if (terminalContainer && terminalContent && commandSpan) {
    const commandText = 'git clone https://github.com/Kemboijames7/portfolio.git';
    ...
    const typeCommand = () => {
        if (i < commandText.length) {
            ...
            cursor.className = `inline-block w-2 h-5 bg-gray-900 
            dark:bg-white ml-1 animate-blink align-middle`;
    ...
    setTimeout(typeCommand, 50);
        ...
        ('afterend', '<span class="animate-blink"></span>');
    }
        };
        //Start typing after a delay
        setTimeout(typeCommand,1000)
